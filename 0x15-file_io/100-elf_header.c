...
/**
 * _strncmp - compare two strings sent
 * @s1: the first strings
 * @s2: second strings
 * @n: the max number of byte to bbe compared
 *
 * Return: zero for equal, other if not
 */
...
/**
 * _close - close a file descriptor and print error message if failure
 ...
 * @buffer: the ELF header
 * @big_endian: endianness if not zero endian
 */
...
/**
 * main - copy a file content to other file
...
int main(int argc, const char *argv[])
{
	...
	big_end = elf_data(buffer);
	elf_version(buffer);
	elf_osabi(buffer);
	elf_abivers(buffer);
	elf_type(buffer, big_end);

	lseek(fd, 24, SEEK_SET);
	_read(fd, (char *) buffer, bit_mode / 8);

	elf_entry(buffer, bit_mode, big_end);
